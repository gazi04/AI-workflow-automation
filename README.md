...
3. Note your **Project ID** (e.g., `my-workflow-automation`)
...
4. Configure:
   - **Name**: Your app name
   - **Authorized redirect URIs**: `http://localhost:8000/api/auth/callback/google` (dev) or `https://your-domain.com/api/auth/callback/google` (prod)
5. Copy the **Client ID** and the **Client Secret** into the `backend/.env` file

### 4. Create Pub/Sub Topic & Setup Local Webhook

1. Go to **Pub/Sub > Topics** and click **Create Topic** (e.g., `gmail-push-notifications`).
2. **Local Development Routing:** Google Cloud requires a public HTTPS URL to deliver push notifications. Because you are testing locally, you must use a tunneling service like [ngrok](https://ngrok.com/) to expose your local FastAPI backend:
   ```bash
   # Expose the backend API port (8000)
   ngrok http 8000
   ```
3. Copy the secure Forwarding URL generated by ngrok (e.g., `https://abc-123.ngrok-free.app`).
4. In the Google Cloud Console, navigate back to your topic and click **Create Subscription**.
5. Set the **Delivery Type** to **Push**.
6. In the **Endpoint URL** field, paste your copied ngrok URL appended with the application's webhook route.
