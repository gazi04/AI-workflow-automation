...
class User(Base):
    ...
    workflow_templates: Mapped[List["WorkflowTemplate"]] = relationship(back_populates="created_by_user")
...
class WorkflowRun(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('success', 'failure', 'running')"),
    )
...
class UserSettings(Base):
    ...
    notification_preferences: Mapped[dict] = mapped_column(JSONB, default={"email": True, "slack": False})
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow
    )
