...
def test_equals_exact_match():
    ...
    condition = make_condition(
        [make_rule("{{trigger.status}}", ConditionOperators.EQUALS, "active")]
    )
...
def test_equals_email_format_extracts_address():
    ...
    condition = make_condition(
        [make_rule("{{trigger.from}}", ConditionOperators.EQUALS, "alice@example.com")]
    )
    assert evaluate_condition(condition, ctx) is True
...
def test_contains_substring_match():
    ...
    condition = make_condition(
        [make_rule("{{trigger.subject}}", ConditionOperators.CONTAINS, "invoice")]
    )
    assert evaluate_condition(condition, ctx) is True
...
def test_contains_case_insensitive():
    ...
    condition = make_condition(
        [make_rule("{{trigger.subject}}", ConditionOperators.CONTAINS, "urgent")]
    )
    assert evaluate_condition(condition, ctx) is True
...
def test_exists_missing_nested_path_returns_false():
    ctx = {"trigger": {}}
    condition = make_condition(
        [make_rule("{{trigger.body.from}}", ConditionOperators.EXISTS, None)]
    )
    assert evaluate_condition(condition, ctx) is False
...
def test_greater_than_true():
    ctx = {"node": {"count": "10"}}
    condition = make_condition(
        [make_rule("{{node.count}}", ConditionOperators.GREATER_THAN, "5")]
    )
    assert evaluate_condition(condition, ctx) is True
...
def test_less_than_true():
    ctx = {"node": {"count": "2"}}
    condition = make_condition(
        [make_rule("{{node.count}}", ConditionOperators.LESS_THAN, "5")]
    )
    assert evaluate_condition(condition, ctx) is True
