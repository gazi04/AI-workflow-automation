...
def test_label_email_color_fields_use_color_widget():
    ...
    catalog = build_catalog()
    label_email = next(a for a in catalog.actions if a.type == "label_email")
    color_fields = [f for f in label_email.fields if f.key in ("background_color", "text_color")]
...
def test_all_four_trigger_types_present():
    catalog = build_catalog()
    types = {t.type for t in catalog.triggers}
    assert {"email_received", "manual", "schedule", "new_sheet_row"} == types
