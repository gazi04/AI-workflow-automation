...
def create_email_payload(
    message_id, labels=["INBOX"], subject="Hello", sender="test@example.com"
):
    return {
        "id": message_id,
        "threadId": "thread-123",
...
def test_process_message_success_trigger(
    ...
    mock_processed_service.create.assert_called_once_with(
        ANY, message_id, matching_workflow.id
    )
...
def test_process_message_skips_wrong_label(mock_db_session, processor, mock_service):
    ...
    email_payload = create_email_payload(
        message_id, labels=["SENT", "IMPORTANT"]
    )  # No INBOX
    mock_service.users.return_value.messages.return_value.get.return_value.execute.return_value = email_payload
    ...
    processor.logger.debug.assert_any_call(
        f"Skipping message {message_id}. Labels ['SENT', 'IMPORTANT'] do not meet criteria (Must be INBOX, not SPAM/TRASH)."
    )
