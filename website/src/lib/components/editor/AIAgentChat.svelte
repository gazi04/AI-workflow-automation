...
<div class="absolute right-6 bottom-6 z-50 flex flex-col items-end gap-4">
	{#if isOpen}
		<div
	...
	{#if !isOpen}
		<Button
			onclick={() => (isOpen = true)}
			size="icon"
			class="h-14 w-14 rounded-full shadow-lg transition-all hover:scale-105 hover:shadow-xl"
		>
			<Bot size={28} />
		</Button>
	{/if}
